  ...
  <div class="row">
    <div :class="'col-' + columnSize">
      <div class="row harness-ui-filtergrid-row">
        <div class="col">
          <label class="col-form-label">Add Filter</label>
          <select
            class="form-control"
            name="filterPicker"
            id="filterPicker"
        ...
        <div class="col text-center">
          <label class="col-form-label">Remove Filter</label>
          <select
            class="form-control"
            name="filterPicker"
            id="filterPicker"
      ...
      <div class="row button-row harness-ui-filtergrid-row harness-ui-filtergrid-buttonrow pt-3">
        <div :class="'col-md-12 text-' + removeButtonPosition">
          <button
            role="button"
            class="btn btn-primary btn-sm harness-ui-filtergrid-clearbutton"
            @click="active=[]; initializeDefaults(); loadData();"
          >
            Remove All Filters
          </button>
        </div>
  ...
  methods: {
    ...
    removeFilter (event) {
      if (event.target.value) {
        ...
        this.setFilter(event.target.value, this.getFilterDefault(event.target.value))
        this.loadData()
      }
    }
  }
